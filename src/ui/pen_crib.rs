...
use crate::{
    BAPViewModel,
    core::{commands::ViewCommand, project::PenDetail},
    view_model::CommandContext,
};
use eframe::egui;
...
pub(crate) fn pen_crib_window(model: &mut BAPViewModel, ctx: &egui::Context) {
    ...
    egui::Modal::new(Id::new("Pen Crib")).show(ctx, |ui| {
        ...
        egui::ScrollArea::vertical()
            .max_height(ctx.available_rect().shrink(128.).height().min(300.))
            .auto_shrink(false)
            .show(ui, |ui| {
                            ...
                            for (idx, pen) in pens.iter_mut().enumerate() {
                                ...
                                let pen_drag_response = ui
                                    .dnd_drag_source(
                                        ...
                                        |ui| {
                                            ui.label(
                                                format!("✋{}✋", pen.name.clone()).to_string(),
                                            );
                                        },
                                    )
                                    .response;
                                if ui
                                    ...
                                    .clicked()
                                {
                                    model.set_command_context(
                                        crate::view_model::CommandContext::PenEdit(
                                            idx,
                                            pen.clone(),
                                        ),
                                    );
                                ...
                                if let (Some(pointer), Some(hovered_payload)) = (
                                    ...
                                    if let Some(dragged_payload) =
                                        pen_drag_response.dnd_release_payload()
                                    {
        ...
        if ui.button("≈Merge matching pens").clicked() {
